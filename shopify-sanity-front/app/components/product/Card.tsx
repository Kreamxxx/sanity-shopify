...
export default function ProductCard({
  ...
  // Sécurisation des données pour éviter l'erreur de lecture sur `undefined.nodes`
  const variantNodes = storefrontProduct?.variants?.nodes ?? [];

  if (variantNodes.length === 0) {
    console.warn('[ProductCard] Aucune variante trouvée pour ce produit :', storefrontProduct);
    return null;
  }

  const firstVariant =
    useGid<ProductVariant>(variantGid) ??
    variantNodes.find((variant) => variant.id == variantGid) ??
    variantNodes[0];

  if (!firstVariant) {
    console.warn('[ProductCard] Aucune variante valide pour :', storefrontProduct);
    return null;
  }
